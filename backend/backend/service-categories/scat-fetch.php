...
if (!empty($keyword)) {
    $sql = "SELECT * FROM service_categories WHERE scat_name LIKE '%{$keyword}%' ORDER BY scat_id DESC LIMIT $start, $perPage";
} else {
    $sql = "SELECT * FROM service_categories ORDER BY scat_id DESC LIMIT $start, $perPage";
}
            ...
            <tbody>
                ...
                while ($row = $result->fetch_assoc()) { ?>
                    <tr>
                        ...
                        <td><?php echo htmlspecialchars($row["scat_description"]); ?></td>
                        <td>
                            <?php $status = $row['scat_status']; ?>
                            <button id="statusButton<?php echo $row['scat_id']; ?>"
                                class="btn btn-<?php echo $status ? 'success' : 'danger'; ?> btn-sm"
                                onclick="toggleStatus(<?php echo $row['scat_id']; ?>, <?php echo $status ? 'false' : 'true'; ?>)">
                                <?php echo $status ? 'เปิด' : 'ปิด'; ?>
                            </button>
                        </td>
                        <td>
                            <button data-toggle="modal" data-target="#IModal" class="btn btn-primary btn-sm" onclick="scatModalEdit('<?php echo $row['scat_id']; ?>','แก้ไขข้อมูล');">แก้ไข</button>
                            <button class="btn btn-danger btn-sm" onclick="scatModalDelete('<?php echo $row['scat_id']; ?>');">ลบ</button>
                        </td>
                    </tr><?php } ?>
            </tbody>
        </table>
        <?php
        $sql = "SELECT * FROM service_categories ORDER BY scat_id DESC";
